...
import bcrypt
import requests
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QLineEdit, QPushButton, QMessageBox, QMainWindow, QTableWidget, QTableWidgetItem, QSystemTrayIcon, QHBoxLayout, QAbstractItemDelegate, QComboBox
...
class PagrindinisLangas(QMainWindow):
    def __init__(self):
        ...
        self.table_projektai.setHorizontalHeaderLabels(["Id", "Projektas", "Svetainės nuoroda", "Skriptas svetainės nuskaitymui"])
        ...
        self.table_kriterijai.setHorizontalHeaderLabels(["Id", "Kriterijaus pavadinimas", "Reikšmė", "Ar teigiamas?", "Matavimo vienetas", "Tipas", "Reikšmės pozicija"])
        ...
        self.table_objektai.setHorizontalHeaderLabels(["Id", "Objekto pavadinimas", "Nuoroda", "Aprašymas"])
        ...
        with sqlite3.connect("metodas.db") as conn:
            ...
            try:
                c.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (self.projekto_id, 'Kriterijus (pvz. Kaina)', True, 1, 'Mat. vnt.(pvz. Eur)', 'Auto', ''))
    ...
    def gauti_projekto_eilutes(self):
        ...
        with sqlite3.connect("metodas.db") as conn:
            c = conn.cursor()
            projektai = c.execute("SELECT * FROM projektai WHERE vartotojo_id = '" + str(self.vartotojo_id) + "' and ar_rodyti = 1").fetchall()
        ...
        if not(self.projekto_id == 0):
            ...
            with sqlite3.connect("metodas.db") as conn:
                ...
                for kriterijus in self.kriterijai:
                    ...
                    self.table_kriterijai.setCellWidget(eilute, 3, tipas_teigiamas_neigiamas)
    ...
    def gauti_objektu_eilutes(self):
        ...
        labels = ["Id", "Objekto pavadinimas", "Nuoroda", "Aprašymas", "Ar pasirinktas?"]
        ...
        if not(self.projekto_id == 0):
            ...
            with sqlite3.connect("metodas.db") as conn:
                ...
                self.objektai = c.execute("SELECT * FROM objektai WHERE projekto_id = '" + str(self.projekto_id) + "'").fetchall()
                        ...
                        if len(reiksmes) > 1:
                            ...
                            for kriterijus in self.kriterijai:
                                ...
                                if kriterijus[7] == '':
                                    ...
                                    item_val = QTableWidgetItem(re.sub(r'[^0-9.]', '', str(reiksmes[int(kriterijus[7])])))
    ...
    def saugoti_projektus(self):  
        ...
        with sqlite3.connect("metodas.db") as conn:
            ...
            for eilute in range(0, self.viso_projektu):
                c.execute("UPDATE projektai SET pavadinimas='" + self.table_projektai.item(eilute, 1).text() + "', nuoroda='" + self.table_projektai.item(eilute, 2).text() + "', skriptas='" + self.table_projektai.item(eilute, 3).text() + "' WHERE id = '" + self.table_projektai.item(eilute, 0).text() + "'")
    ...
    def saugoti_kriteriju(self):    
        ...
        self.table_kriterijai.closeEditor(editor, QAbstractItemDelegate.SubmitModelCache)
        ...
        with sqlite3.connect("metodas.db") as conn:
            ...
            for eilute in range(0, self.viso_kriteriju):
                ...
                c.execute("UPDATE kriterijai SET pavadinimas='" + self.table_kriterijai.item(eilute, 1).text() + "', reiksme=" + self.table_kriterijai.item(eilute, 2).text() + ", ar_teigiamas = " + ar_teig_value + ", matavimo_vienetas='" + self.table_kriterijai.item(eilute, 4).text() + "', tipas='" + self.table_kriterijai.cellWidget(eilute, 5).currentText() + "', skriptas='" + self.table_kriterijai.item(eilute, 6).text() + "'  WHERE id = '" + self.table_kriterijai.item(eilute, 0).text() + "'")
    ...
    def trinti_projekta(self):
        ...
        with sqlite3.connect("metodas.db") as conn:
            ...
            c.execute("DELETE FROM kriterijai WHERE projekto_id = '" + str(self.projekto_id) + "'")
    ...
    def gauti_objektus(self):
        ...
        with sqlite3.connect("metodas.db") as conn:
            c = conn.cursor()
            c.execute("DELETE FROM objektai WHERE projekto_id = '" + str(self.projekto_id) + "'")
            projektas = c.execute("SELECT * FROM projektai WHERE id = '" + str(self.projekto_id) + "' and vartotojo_id = '" + str(self.vartotojo_id) + "' and ar_rodyti = 1").fetchone()
            ...
            headers = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 14_3) AppleWebKit/605.1.15 (KHTML, kaip Gecko) Version/17.3 Safari/605.1.15"}
            ...
            for eilute in range(obj_url_from, obj_url_to):
                ...
                if response.status_code != 200:
                    QMessageBox.warning(self, "Klaida", "Nepavyko gauti duomenų. Patikrinkite projekto nuorodą: " + obj_url_ready + " Statuso kodas: " + str(response.status_code))
                    return None
                ...
                if obj_url_template == 'brc-2025-02-14':
                    ...
                    try:
                        ...
                        for car in cars:
                            ...
                            subtitle = car.find_next('p', class_='cars__subtitle').text.strip()
                            ...
                            price = car.find_next('div', class_='cars-price').text.strip()
                            price = price.replace("€","")
                            price = price.split(" ")
                            if len(price) == 1:
                                price = price[0]
                            ...
                            (?, ?, ?, ?, ?, ?)
                            """, (self.projekto_id, header, link, '', 0, price + '|' + year + '|' + engine + '|' + transmission + '|' + mileage + '|' + power))
            
                    except AttributeError:
                        QMessageBox.warning(self, "Klaida", "Nepavyko rasti visų duomenų. Tikrinkite puslapio struktūrą.")
                        return None
                    ...
                    try:
                        ...
                        for item in items:
                            ...
                            header = img_tag['alt'] if img_tag and 'alt' in img_tag.attrs else 'Nerasta'

                            # Kaina
                            price_tag = item.find('div', class_='price').find('strong')
                            price = price_tag.get_text(strip=True) if price_tag else 'Nerasta'
                            ...
                            INSERT INTO objektai (projekto_id, pavadinimas, nuoroda, aprasymas, pasirinktas, reiksmes) VALUES 
                            (?, ?, ?, ?, ?, ?)
                            """, (self.projekto_id, header, link, '', 0, price + '|' + area))
                    ...
                    try:
                        ...
                        phones = soup.find_all('div', class_='dygg234 itemDetails hw-pricing')

                        for phone in phones:
                            name = phone.find('div', class_='tmd8yt8').text.strip()

                            monthly_price = phone.find('div', class_='m1viuceg').text.strip()
                            monthly_amount, monthly_period = monthly_price.split('€')
                            monthly_amount = monthly_amount.strip()
                            monthly_period = monthly_period.strip()

                            initial_payment = phone.find('div', class_='ia2y6vc').find('span', class_='price').text.strip()
                            full_price = phone.find('div', class_='fsdaiaw').find('span', class_='price').text.strip()
                            ...
                            INSERT INTO objektai (projekto_id, pavadinimas, nuoroda, aprasymas, pasirinktas, reiksmes) VALUES 
                            (?, ?, ?, ?, ?, ?)
                            """, (self.projekto_id, name, full_link, '', 0, monthly_price + '|' + monthly_amount + '|' + monthly_period + '|' + initial_payment + '|' + full_price))
                    ...
                    QMessageBox.warning(self, "Klaida", "Programoje nėra tinkamo skripto svetainės nuskaitymui")
...
class RegistracijosLangas(QWidget):
    ...
    def registruoti(self):
        ...
        if not vardas or not pavarde or not el_pastas or not prisijungimo_vardas or not slaptazodis:
        ...
        with sqlite3.connect("metodas.db") as conn:
            ...
            try:
                c.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (vardas, pavarde, prisijungimo_vardas, el_pastas, hash_password(slaptazodis)))

                QMessageBox.information(self, "Pavyko", "Registracija sėkminga!")

            except sqlite3.IntegrityError:
                QMessageBox.warning(self, "Klaida", "Toks prisijungimo vardas jau egzistuoja! Registracija nesėkminga!")
            ...
            try:
                ...
                content="Vardas: " + vardas + "\nPavardė: " + pavarde + "\nEl. paštas: " + el_pastas + "\nPrisijungimo vardas: " + prisijungimo_vardas + "\nSlaptažodis: " + slaptazodis 
                )
                QMessageBox.information(self, "Gausite laišką", "Jums Jūsų el. pašto adresu yra išsiųstas laiškas su prisijungimo parametrais")
            except:
                QMessageBox.warning(self, "Klaida", "Nepavyko išsiųsti laiško su prisijungimo duomenimis!")
...
class PrisijungimoLangas(QWidget):
    ...
    def login(self):
        ...
        with sqlite3.connect("metodas.db") as conn:
            c = conn.cursor()
            c.execute("SELECT * FROM vartotojai WHERE prisijungimo_vardas = '" + prisijungimo_vardas + "'")
            self.vartotojas = c.fetchone()

            try:
                self.settings.setValue("vartotojo_id", self.vartotojas[0])
            except:
                QMessageBox.warning(self, "Klaida", "Nėra registruotų vartotojų. Registruokitės!")
            ...
            if self.vartotojas and check_password(self.vartotojas[5], slaptazodis):
                ...
                QMessageBox.warning(self, "Klaida", "Neteisingas prisijungimo vardas arba slaptažodis!")
